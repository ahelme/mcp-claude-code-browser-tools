...
/**
 * MCP Server for Browser Tools - 2025-03-26 Specification Compliant
 * 
 ...
 * - Interfaces with browser-tools HTTP server on port 3025
 * 
...
function debugLog(message) {
    if (process.env.MCP_DEBUG) {
        process.stderr.write(`[browser-tools-mcp] ${new Date().toISOString()} - ${message}\n`);
    }
}
...
async function callBrowserTools(action, params) {
    return new Promise((resolve, reject) => {
        const port = process.env.BROWSER_TOOLS_PORT || '3025';
        ...
        const req = http.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    debugLog(`Browser-tools response: ${JSON.stringify(result).substring(0, 200)}`);
                    ...
                    reject(new Error(`Invalid response from browser-tools: ${e.message}`));
                }
            });
        });
        ...
        req.on('error', (e) => {
            ...
            reject(new Error(`Browser-tools server not available on port ${port}: ${e.message}`));
        });
...
async function handleRequest(request) {
    ...
    try {
        switch (method) {
            case 'initialize':
                // Capability negotiation per 2025-03-26 spec
                const { protocolVersion: clientVersion, capabilities: clientCaps } = params;
                
                // Check protocol compatibility
                if (clientVersion && !clientVersion.startsWith('2025')) {
                    debugLog(`Protocol version mismatch: client ${clientVersion}, server ${serverInfo.protocolVersion}`);
                }
            ...
            case 'tools/call':
                ...
                try {
                    ...
                    // Format response per 2025-03-26 spec
                    const response = {
                        content: []
                    };
                    ...
                    if (result.screenshot) {
                        ...
                        response.content.push({
                            type: "text",
                            text: result.html || result.text || JSON.stringify(result, null, 2)
                        });
